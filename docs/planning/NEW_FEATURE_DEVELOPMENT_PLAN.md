...
| 顺序 | 任务范围 | 放行条件 |
|---|---|---|
| P0-A | B09–B16 真实浏览器人工验收 | 原生输入法、键盘、窗口和恢复路径有具名记录；阻断问题关闭 |
| P0-B | F02-6、F03-7、F04-7、F05-5 真实效果评估 | 至少两名独立评审者；分别报告命中、误报、遗漏、修订改善和等待 |
| P1 | F06-1～F06-5 本地选题箱 | SQLite 持久化、幂等迁移、搜索/继续写、备份恢复和 B17 通过 |
| P2 | F07 候选稿对照 | 真实使用出现保留两稿比较的需求后进入规格 |
| P3 | F08 阶段 A | F04 判断质量通过，且材料准备是已观察到的瓶颈 |
| P4 | F09 阶段 A | F05 定位有效，候选隔离可用，且有调整结构顺序的真实需求 |

P0 评审材料发出后可推进 P1，不必等待人工回收期间停工。P2–P4 按证据选择一项，不能默认连续开发。
